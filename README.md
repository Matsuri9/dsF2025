...
├── code/
│   ├── utils.py                  # CoNLL-Uパース・タグマージ等の共通ユーティリティ
│   ├── data_loader.py            # データ読み込みモジュール（JSON読み込み、言語マッピング）
│   ├── head_direction.py         # Head Direction分析モジュール（ペア抽出・距離計算）
│   ├── word_order.py             # 語順分析モジュール（レーベンシュタイン距離・n-gram）
│   ├── visualization.py          # 可視化モジュール（ヒートマップ・樹形図・MDS等）
│   ├── parse_conllu.py           # CoNLL-U → JSON変換スクリプト
│   ├── calculate_head_direction.py  # Head Direction分析実行スクリプト
│   ├── analyze_word_order.py     # 語順分析実行スクリプト
│   ├── merge_to_phrases.py       # 句単位へのマージ処理スクリプト
│   └── extract_zips.py           # データ解凍スクリプト
├── data/
│   ├── raw/                      # 生のUD CoNLL-Uファイル
│   ├── processed/                # 変換済みJSONデータ
│   ├── phrases/                  # 句単位データ
│   └── results/                  # 分析結果 (CSV, 可視化画像)
├── docs/                         # ドキュメント・進捗レポート
...
```
              ...
              ▼              ▼              ▼
      ┌───────────────┐ ┌───────────────┐ ┌───────────────┐
      │ data_loader.py│ │head_direction │ │ visualization │
      │               │ │     .py       │ │     .py       │
      └───────────────┘ └───────────────┘ └───────────────┘
              │              │              │
              │              │              │
              ▼              ▼              ▼
    ┌─────────────────────────────────────────────────────┐
    │         実行スクリプト（calculate_head_direction.py等）│
    └─────────────────────────────────────────────────────┘
```
