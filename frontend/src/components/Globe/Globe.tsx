...
import { LANGUAGE_LOCATIONS, getLanguageByName } from '../../data/languageLocations';
...
interface ArcData {
    ...
    distance: number;
    geoDistance: number;
    category: 'veryClose' | 'close' | 'slightlyClose' | 'slightlyFar' | 'far' | 'veryFar' | 'same';
    clickable: boolean;
    node1Code: string;
    node2Code: string;
}
...
function getDistanceCategory(distance: number): 'veryClose' | 'close' | 'slightlyClose' | 'slightlyFar' | 'far' | 'veryFar' {
...
export function GlobeVisualization({ similarityData, onLanguageClick, distanceFilter, selectedNodeCode }: Props) {
    ...
    const pointsData: PointData[] = useMemo(() => {
        return LANGUAGE_LOCATIONS.map(lang => ({
            ...
            color: selectedNodeCode === lang.code ? '#22c55e' : (lang.isPrimary ? '#60a5fa' : '#94a3b8'),
    ...
    const arcsData: ArcData[] = useMemo(() => {
        ...
        for (let i = 0; i < allNodes.length; i++) {
            for (let j = i + 1; j < allNodes.length; j++) {
                ...
                if (node1.primaryCode === node2.code || node2.primaryCode === node1.code ||
                    (node1.primaryCode && node1.primaryCode === node2.primaryCode)) {

                    const geoDistance = calculateGeoDistance(node1.lat, node1.lng, node2.lat, node2.lng);
        ...
        for (let i = 0; i < allNodes.length; i++) {
            for (let j = i + 1; j < allNodes.length; j++) {
                ...
                const lang1Name = node1.isPrimary ? node1.name :
                    LANGUAGE_LOCATIONS.find(l => l.code === node1.primaryCode)?.name;
                const lang2Name = node2.isPrimary ? node2.name :
    ...
    return (
        <div className="globe-container">
            <Globe
                ...
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                pointLabel={(d: any) => {
                    return '<div class="globe-tooltip"><div class="tooltip-title">' + d.nameJa + '</div><div class="tooltip-subtitle">' + d.country + '</div></div>';
                }}
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                ...
                arcAltitude={(d: any) => {
                    ...
                    return arc.category === 'same' ? 0.01 : getArcAltitude(arc.geoDistance);
                }}
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                arcStroke={(d: any) => {
                    const arc = d as ArcData;
                ...
                arcLabel={(d: any) => {
                    ...
                    return '<div class="globe-tooltip arc-tooltip"><div class="tooltip-title">' + d.lang1 + ' ↔ ' + d.lang2 + '</div><div class="tooltip-distance">距離: ' + d.distance.toFixed(4) + '</div><div class="tooltip-hint">クリックで詳細比較</div></div>';
                }}
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                onArcClick={(arc: any) => handleArcClick(arc as ArcData)}
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                onArcHover={(arc: any) => setHoverArc(arc?.clickable ? arc as ArcData : null)}
            ...
            {hoverArc && (
                <div className="arc-info-panel">
                    <span className="arc-languages">{hoverArc.lang1} ↔ {hoverArc.lang2}</span>
                    <span className="arc-distance">距離: {hoverArc.distance.toFixed(4)}</span>
                </div>
            )}
        </div>
    );
