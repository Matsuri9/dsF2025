...
export function LanguageSimilarityPanel({ selectedNodeCode, similarityData, onClose }: Props) {
    const navigate = useNavigate();

    const selectedNode = LANGUAGE_LOCATIONS.find(l => l.code === selectedNodeCode);
    ...
    const sortedSimilarities = useMemo(() => {
        ...
        const selectedLangName = selectedNode.isPrimary
            ? selectedNode.name
            : LANGUAGE_LOCATIONS.find(l => l.code === selectedNode.primaryCode)?.name;
        ...
        for (const node of LANGUAGE_LOCATIONS) {
            ...
            if (selectedNode.primaryCode && node.code === selectedNode.primaryCode) continue;
            if (selectedNode.primaryCode && node.primaryCode === selectedNode.primaryCode) continue;
    ...
    const handleItemClick = (item: SimilarityItem) => {
        ...
        const lang1 = getLanguageByName(selectedNode.isPrimary ? selectedNode.name :
            LANGUAGE_LOCATIONS.find(l => l.code === selectedNode.primaryCode)?.name || '');
            ...
            <div className="similarity-header">
                <div className="similarity-title-section">
                    ...
                    <p className="similarity-subtitle">
                        {selectedNode?.nameJa} との距離
                    </p>
                </div>
                <button className="similarity-close" onClick={onClose}>×</button>
            </div>
