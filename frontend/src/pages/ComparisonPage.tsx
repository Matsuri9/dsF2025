...
export function ComparisonPage() {
  const { lang1: urlLang1, lang2: urlLang2 } = useParams<{
    lang1?: string;
    lang2?: string;
  }>();
  ...
  const lang1Location = LANGUAGE_LOCATIONS.find(
    (l) => l.code === selectedLang1,
  );
  const lang2Location = LANGUAGE_LOCATIONS.find(
    (l) => l.code === selectedLang2,
  );
  ...
  const deprelSimilarity =
    similarityData.deprel[lang1Full]?.[lang2Full] ?? null;
  const phraseSimilarity =
    similarityData.phrase[lang1Full]?.[lang2Full] ?? null;
  const headDirectionRawSimilarity =
    similarityData.headDirectionRaw[lang1Full]?.[lang2Full] ?? null;
  const headDirectionMergedSimilarity =
    similarityData.headDirectionMerged[lang1Full]?.[lang2Full] ?? null;

  // Get sentences for selector
  const sentences =
    lang1Data && lang2Data
      ? lang1Data.sentences.slice(
          0,
          Math.min(lang1Data.sentences.length, lang2Data.sentences.length),
        )
      : [];
  ...
  // ローディング中の表示
  const isLoading =
    (selectedLang1 && isLanguageLoading(selectedLang1)) ||
