...
import type { LanguageData, PhraseData, SimilarityData, SimilarityMatrix, HeadDirectionRates } from '../types';
import { LANGUAGE_NAMES, PROCESSED_PATH, PHRASES_PATH, RESULTS_PATH } from '../constants';
...
function parseHeadDirectionRatesCSV(csvText: string): HeadDirectionRates {
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        for (let j = 0; j < languageHeaders.length; j++) {
            ...
            rates[key][lang] = value && value.trim() !== '' ? parseFloat(value) : null;
        }
    }
...
export function useLazyLanguageData() {
    const [languageData, setLanguageData] = useState<{ [key: string]: LanguageData }>(languageDataCache);
    const [phraseData, setPhraseData] = useState<{ [key: string]: PhraseData }>(phraseDataCache);
    const [loadingLanguages, setLoadingLanguages] = useState<Set<string>>(new Set());
    const [error, setError] = useState<string | null>(null);

    // 言語データを読み込む（キャッシュがあればスキップ）
    const loadLanguage = useCallback(async (code: string): Promise<boolean> => {
        // 既にキャッシュにある場合はスキップ
        ...
        try {
            ...
            let phraseResponse = await fetch(`${PHRASES_PATH}/${code}_pud_phrases.json`);
    ...
    // 言語が読み込み中かどうかを確認
    const isLanguageLoading = useCallback((code: string): boolean => {
    ...
    useEffect(() => {
        async function loadData() {
            ...
            try {
                ...
                for (const code of languageCodes) {
                    try {
                        const response = await fetch(`${PHRASES_PATH}/${code}_pud_phrases.json`);
                        if (response.ok) {
                            const data = await response.json();
                            phData[code] = data;
                        }
        ...
        async function loadData() {
            ...
            try {
                ...
                if (headDirMergedResponse.ok) {
                    data.headDirectionMerged = parseCSV(await headDirMergedResponse.text());
                }
...
export function useHeadDirectionRates() {
    ...
    useEffect(() => {
        async function loadData() {
            try {
                const response = await fetch(`${RESULTS_PATH}/head_direction_rates_merged.csv`);
                if (response.ok) {
